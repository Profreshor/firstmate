...
config/herdr-presentation-spaces  optional Herdr visual layout: "off" opts out of, "on" opts into the default-on disposable single-task projection, and "crew" opens task panes in a shared crew workspace; the unconfigured default requires a Herdr version floor; LOCAL, gitignored; inherited by secondmate homes; see docs/herdr-backend.md "Presentation spaces" and "Crew workspace"
