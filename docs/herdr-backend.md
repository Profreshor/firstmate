...
The value `crew` selects the shared per-home crew workspace owned by [Crew workspace](#crew-workspace) instead of one workspace per task.
...
A home that writes `crew` into `config/herdr-presentation-spaces` keeps its own workspace for the Firstmate agent alone and places every new crewmate or scout, and every reclaim of a gone endpoint, as a split pane in one durable per-home crew workspace labeled `<home-label>-crew`.
`crew:<n>` sets the per-tab pane cap to any value from 1 to 16; plain `crew` means `crew:6`.
...
The crew workspace is created on first use with focus preserved and ordered immediately after the home's workspace on a best-effort basis.
Its id is recorded in `state/.herdr-crew-workspace-<session>`, and a recorded id is reused only while that workspace still exists with the expected crew label, so a relabeled or recycled workspace is never adopted and a label alone never selects placement.
A placement takes the first crew tab holding fewer panes than the cap, and opens a new tab in the same crew workspace when every tab is full.
...
After every split and every crew pane cleanup, Firstmate evens out that tab's split ratios with Herdr's `layout.set_split_ratio`, which resizes panes without restarting them, and never sends `layout.apply`, which recreates every pane.
A later placement into a freed slot fills the gap the closed pane left.
When a whole column closes while the other columns still hold two rows, the remaining columns are evened out and each later pane splits the largest pane along its longer side, so that tab stays even but may not return to the exact 3 by 2 shape until its panes turn over.
...
Cleanup closes only the task's exact pane through the ordinary exact-identity kill path and its focus-safe removal plan, so the crew workspace disappears with its last pane and is recreated at the next placement.
...
- Crew grid rebalancing needs Python 3 and Herdr's layout methods, and a whole-column close leaves that crew tab even but not always in its exact grid shape.
